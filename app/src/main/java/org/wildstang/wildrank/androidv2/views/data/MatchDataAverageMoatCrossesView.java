 ...
 */
public class MatchDataAverageMoatCrossesView extends MatchDataView implements IMatchDataView {


    public MatchDataAverageMoatCrossesView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
...
//                .subscribe(sum -> setValueText("" + sum), error -> Log.d("wildrank", this.getClass().getName()));
        ...
        for (Document document: documents) {
            ...
            int cross = (boolean) data.get("defense-moat") == true ? 1 : 0;
            matches += cross;//Number of Matches?
            moatCrosses += (int) data.get("teleop-moat");
        }

        if (moatCrosses == 0 && matches == 0) {
            setValueText("N/A");
        } else {
            double averageCrosses = (moatCrosses / (matches));
