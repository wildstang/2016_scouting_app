 ...
 */
public class MatchDataAverageDrawbridgeCrossesView extends MatchDataView implements IMatchDataView {


    public MatchDataAverageDrawbridgeCrossesView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
...
//                .subscribe(sum -> setValueText("" + sum), error -> Log.d("wildrank", this.getClass().getName()));
        ...
        for (Document document: documents) {
            ...
            int cross = (boolean) data.get("defense-drawbridge") == true ? 1 : 0;
            matches += cross;//Number of Matches?
            drawbridgeCrosses += (int) data.get("teleop-drawbridge");
        }

        if (drawbridgeCrosses == 0 && matches == 0) {
            setValueText("N/A");
        } else {
            double averageCrosses = (drawbridgeCrosses / (matches));
